...
export const MainActivity = (): JSX.Element =>
{
	...
	const [currAccess, updateAccess] = useState<AccountManager | null>(null);


	const ARRAY_VIEWS: (() => JSX.Element)[] =
	[
		() => (<ViewStore />),
		() => (<ViewAccess account={currAccess} onChangeAccount={updateAccess} />),
	];
		...
		<header class="w-100 bg-body-secondary" style="min-height: 80px;" >
			<AppNavigationMenu
				...
				navButtons={[
					(<ViewStoreButton
						onClick={() => setView(0)} />),
					(<ViewAccountButton account={currAccess}
						onClick={() => setView(1)} />)
			]} />
		</header>
